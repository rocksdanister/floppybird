...
GLuint dlist[6];
int birdPhys,i,j,tmp,highscore,insidebrick,gameover;
double syncBird;
...
glNewList(dlist[0], GL_COMPILE);
	glBegin(GL_QUADS);
		...
		glTexCoord2f(1.0, 0.0); glVertex3f(40,resY/2,0);
		glTexCoord2f(1.0, -1.0); glVertex3f(40,(resY/2)+40,0);                        
		glTexCoord2f(0.0, -1.0); glVertex3f(0,(resY/2)+40,0);
	glEnd();
glEndList();
}
...
glNewList(dlist[4], GL_COMPILE);
	glBegin(GL_QUADS);
		...
		glTexCoord2f(1.0, -1.0); glVertex3f(resX+50,resY/2-27,0);                        
		glTexCoord2f(0.0, -1.0); glVertex3f(resX,resY/2-27,0);
	glEnd();
...
void hitDetection()
{
	...
	{		
		...
		if( (resX+obstacleBrick[i].objdisp)>=0 && (resX+50+obstacleBrick[i].objdisp)<=100 )
		{
			...
			if( (movementY+(resY/2)+8) >= ( ((resY/2)-27)+ obstacleBrick[i].y) && (movementY+(resY/2)+40) <= ( ((resY/2)+27) +obstacleBrick[i].y) )
			{
				hit1++;				
	 		}
			else 
